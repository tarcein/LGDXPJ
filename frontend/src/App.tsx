...
function ScheduleTimeFields({ date, start, end, onDate, onStart, onEnd }: {
  ...
  return <><label className="form-label">날짜</label><input className="form-control" type="date" value={date} onChange={event => onDate(event.target.value)} /><div className="time-pair direct-time"><label><span>시작 시간</span><TimeWheelPicker value={start} onChange={onStart} /></label><label><span>종료 시간 (선택)</span><TimeWheelPicker value={end} onChange={onEnd} /></label></div><p className="time-input-help">퇴근처럼 한 시점의 일정은 종료 시간을 비워두세요.</p></>
}

function TimeWheelPicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const [hour = '', minute = ''] = value.split(':')
  const change = (nextHour: string, nextMinute: string) => onChange(nextHour && nextMinute ? `${nextHour}:${nextMinute}` : '')
  return <div className="time-wheel" aria-label="시간 선택">
    <select value={hour} onChange={event => change(event.target.value, minute)} aria-label="시">
      <option value="">시</option>{Array.from({ length: 24 }, (_, index) => { const item = String(index).padStart(2, '0'); return <option key={item} value={item}>{item}</option> })}
    </select>
    <b>:</b>
    <select value={minute} onChange={event => change(hour, event.target.value)} aria-label="분">
      <option value="">분</option>{Array.from({ length: 60 }, (_, index) => { const item = String(index).padStart(2, '0'); return <option key={item} value={item}>{item}</option> })}
    </select>
  </div>
}
  ...
  return <div className="app-shell"><aside className="screen-index"><div className="brand"><span className="brand-mark">LG</span><div><strong>Family Care</strong><small>기능 목업 개발 버전</small></div></div><p className="index-intro">Figma 기능 페이지의 주요 흐름을 화면별로 확인할 수 있어요.</p>{groups.map(g => <div key={g.title} className="index-group"><h2>{g.title}</h2>{g.pages.map(([id, label]) => <button key={id} className={screen === id ? 'active' : ''} onClick={() => go(id)}>{label}</button>)}</div>)}</aside>
    ...
    {scheduleSheet === 'FORM' && boot && !editingSchedule && scheduleEntryMode === 'SINGLE' && <div className="single-schedule-overlay"><section className="single-schedule-screen" role="dialog" aria-modal="true" aria-label="일정 등록" data-figma-node="460:3102">
      ...
      <div className="single-schedule-card">
        ...
        <div className="single-time direct-time"><label><span>시작</span><TimeWheelPicker value={scheduleStartTime} onChange={setScheduleStartTime} /></label><label><span>종료</span><TimeWheelPicker value={scheduleEndTime} onChange={setScheduleEndTime} /></label></div>
      </div>
    ...
    {scheduleSheet === 'FORM' && boot && !editingSchedule && scheduleEntryMode === 'REPEAT' && <div className="routine-screen-overlay"><section className="routine-screen" role="dialog" aria-modal="true" aria-label="루틴 등록" data-figma-node="557:1551">
      ...
      <div className="routine-form-card">
        ...
        <div className="routine-time-row"><strong>시간</strong><div className="routine-time-fields direct-time"><TimeWheelPicker value={scheduleStartTime} onChange={setScheduleStartTime} /><span>–</span><TimeWheelPicker value={scheduleEndTime} onChange={setScheduleEndTime} /></div></div>
