...
const groups: { title: string; pages: [Screen, string][] }[] = [
  ...
  { title: '가족 · 설정', pages: [['onboarding', '온보딩'], ['calendar', '캘린더 연동 및 루틴 수정'], ['members', '가족 구성원'], ['childProfile', '아이 프로필 수정'], ['permissions', '정보 공개 권한'], ['album', '모음ZIP'], ['settings', '알림 설정'], ['plan', '플랜 비교']] },
...
type OnboardChild = { name: string; birthDate: string; institution: string; photo: File | null }
type KakaoSdk = {
...
const ageFromBirthDate = (birthDate: string) => {
  ...
  return Number.isFinite(age) && age >= 0 ? `만 ${age}세` : ''
}
...
function App() {
  ...
  const [childProfileBirthDate, setChildProfileBirthDate] = useState('')
  const [childProfileInstitution, setChildProfileInstitution] = useState('')
  ...
  const [exceptionScheduleDate, setExceptionScheduleDate] = useState(() => new Date().toLocaleDateString('sv-SE'))
  ...
  const [childBirthDateInput, setChildBirthDateInput] = useState('')
  const [childInstitutionInput, setChildInstitutionInput] = useState('')
  ...
  useEffect(() => {
    ...
    setBillingOrder(null); setBillingWidgetReady(false)
    send<BillingOrder>('/billing/orders', 'POST', { cycle: billingCycle })
      .then(async order => {
        if (cancelled) return
  ...
  }, [billingOpen, billingCycle, screen, subscription?.status])
  useEffect(() => {
  ...
  const startFamilyOnboarding = () => {
    ...
    setOnboardChildren([])
    setChildNameInput('')
    setChildBirthDateInput('')
    setChildInstitutionInput('')
  ...
  const addOnboardChild = () => {
    if (!childNameInput.trim() || !childBirthDateInput) { setError('아이 이름과 생년월일을 입력해주세요.'); return }
    setError('')
    setOnboardChildren(current => [...current, { name: childNameInput.trim(), birthDate: childBirthDateInput, institution: childInstitutionInput.trim(), photo: childPhotoInput }])
    setChildNameInput(''); setChildBirthDateInput(''); setChildInstitutionInput(''); setChildPhotoInput(null); setChildPhotoPreview('')
  }
  const saveOnboardChildren = async () => {
    if (onboardBusy) return
    const pending = [...onboardChildren]
    if (childNameInput.trim() || childBirthDateInput || childInstitutionInput.trim()) {
      if (!childNameInput.trim() || !childBirthDateInput) { setError('작성 중인 아이의 이름과 생년월일을 모두 입력해주세요.'); return }
      pending.push({ name: childNameInput.trim(), birthDate: childBirthDateInput, institution: childInstitutionInput.trim(), photo: childPhotoInput })
    }
    setOnboardBusy(true); setError('')
    try {
      for (const childDraft of pending) {
        const created = await send<Child>('/children', 'POST', { name: childDraft.name, birth_date: childDraft.birthDate, institution: childDraft.institution })
        if (childDraft.photo) {
          const form = new FormData(); form.append('file', childDraft.photo)
          await upload('/children/' + created.id + '/photo', form)
        }
      }
      setOnboardChildren([]); setChildNameInput(''); setChildBirthDateInput(''); setChildInstitutionInput(''); setChildPhotoInput(null); setChildPhotoPreview('')
      await load()
  ...
  const closeScheduleEdit = () => {
    setEditingSchedule(null)
    setScheduleSheet('NONE')
    if (scheduleReturnToWeekly) setWeeklyTimetableOpen(true)
    else if (screen !== 'calendar') setScheduleSheet('DAY')
    setScheduleReturnToWeekly(false)
  }
  ...
  const saveSchedule = (updateScope?: 'SINGLE' | 'FUTURE', mergeSameLocation?: boolean) => {
    ...
    if (editingSchedule) {
      if (editingSchedule.type === 'CHILD') {
        ...
        else { setSelectedDate(scheduleDate); closeScheduleEdit() }
        return
      } else {
      ...
      setScheduleTitle(''); setSelectedDate(scheduleDate); closeScheduleEdit()
      return
    }
  ...
  const openChildProfile = (target: Child) => {
    const legacyInstitution = target.age_label.includes('·') ? target.age_label.split('·').slice(1).join('·').trim() : ''
    setChildProfileId(target.id); setChildProfileName(target.name); setChildProfileBirthDate(target.birth_date ?? ''); setChildProfileInstitution(target.institution || legacyInstitution); go('childProfile')
  }
  const saveChildProfile = () => {
    if (!childProfileId || !childProfileName.trim() || !childProfileBirthDate) { setError('아이 이름과 생년월일을 입력해주세요'); return }
    void run(
      () => send('/children/' + childProfileId, 'PATCH', { name: childProfileName.trim(), birth_date: childProfileBirthDate, institution: childProfileInstitution.trim() }),
      '아이 프로필을 변경했어요',
    )
  }
  ...
  if (boot && screen === 'schedule') {
    ...
    page = <section className="figma-calendar-page" data-figma-node="702:1663">
      <button className="figma-calendar-connect" onClick={() => go('calendar')}><i><img src={googleIcon} alt="" /><img src={outlookIcon} alt="" /></i><span><strong>개인 캘린더 연동 및 루틴 수정</strong><small>{connectedCalendarCount ? `${connectedCalendarCount}개 계정 연결됨 · 개인 루틴 ${personalRoutineGroups.length}개` : `캘린더 연결 · 개인 루틴 ${personalRoutineGroups.length}개 확인`}</small></span><b>›</b></button>
  ...
  if (boot && screen === 'exception') page = <><div className="eyebrow urgent">EXCEPTION CARE</div><h2 className="hero-title one-line">{activeExceptions.length ? '지금 확인이 필요해요' : '현재 예외 상황이 없어요'}</h2>{activeExceptions.length ? <Card className="urgent-card"><span className="small-badge danger">예외 상황 {activeExceptions.length}건</span><strong>{activeExceptions[0].reason}</strong><p>진행 중인 대안을 확인하고 담당자를 조정해 주세요.</p></Card> : <Card className="urgent-card safe"><span className="small-badge ok">정상</span><strong>감지된 일정 충돌이 없어요.</strong><p>가족 일정이 겹치면 이 화면에서 바로 알려드릴게요.</p></Card>}<Section>진행 중인 대안</Section>{activeExceptions.length ? activeExceptions.map(e => <Card key={e.id} className="exception-card"><strong>{e.reason}</strong><p>대안 · {member(e.alternative_member_id)}</p><span className="small-badge danger">확인 대기</span><button className="primary-button" onClick={() => run(() => send('/exceptions/' + e.id + '/approve', 'POST'), '대안을 요청했어요')}>대안 승인</button></Card>) : <Empty title="새로운 대안이 없어요" text="충돌이 생기면 해결책을 이곳에서 확인할 수 있어요" />}<Section>직접 대안 제안</Section><Card className="form-card"><label className="form-label">변경할 날짜</label><input type="date" className="form-control" min={todayKey} value={exceptionScheduleDate} onChange={e => { setExceptionScheduleDate(e.target.value); setAssignmentId(null) }} /><label className="form-label">변경할 일정</label><select className="form-control" value={selectedExceptionAssignment?.id ?? ''} disabled={!exceptionAssignmentOptions.length} onChange={e => setAssignmentId(e.target.value)}><option value="">{exceptionAssignmentOptions.length ? '일정을 선택하세요' : '선택한 날짜에 확정된 배정이 없어요'}</option>{exceptionAssignmentOptions.map(assignment => { const item = itemFor(assignment); return item && <option key={assignment.id} value={assignment.id}>{formatDate(item.starts_at)} · {formatTime(item.starts_at)} · {child(item.child_id)} · {item.title} · 담당 {member(assignment.assignee_id)}</option> })}</select><label className="form-label">다른 담당자</label><select className="form-control" value={alternative} onChange={e => setAlternative(e.target.value)}>{members.filter(m => m.id !== me?.member.id).map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select><label className="form-label">사유</label><input className="form-control" value={reason} onChange={e => setReason(e.target.value)} /><button className="primary-button wide-button" disabled={!selectedExceptionAssignment} onClick={() => run(async () => { if (!exceptionScheduleDate) throw new Error('날짜를 선택해주세요'); if (!selectedExceptionAssignment) throw new Error('일정을 선택해주세요'); await send('/exceptions', 'POST', { assignment_id: selectedExceptionAssignment.id, alternative_member_id: alternative, reason }) }, '대안을 등록했어요')}>대안 만들기</button></Card></>
  ...
  if (screen === 'onboarding' && !invitationFromUrl && onboardStep === 'CHILD') page = <div className="onboard-flow"><div className="onboard-step-head"><button aria-label="역할 선택으로 돌아가기" onClick={() => setOnboardStep('ROLE')}>‹</button><div className="onboard-step-track"><span className="active" /><span className="active" /><span className="active" /><span /></div><small>3/4</small></div><h2 className="hero-title">아이 정보를 알려주세요</h2>{onboardChildren.length > 0 && <div className="onboard-child-list">{onboardChildren.map((kid, index) => <Card key={`${kid.name}-${index}`}><span><strong>{kid.name}</strong><small>{ageFromBirthDate(kid.birthDate)}{kid.institution ? ` · ${kid.institution}` : ''}{kid.photo ? ' · 사진 등록됨' : ''}</small></span><button aria-label={`${kid.name} 삭제`} onClick={() => setOnboardChildren(current => current.filter((_, itemIndex) => itemIndex !== index))}>×</button></Card>)}</div>}<Card className="form-card child-onboard-card"><label className="child-photo-picker">{childPhotoPreview ? <img src={childPhotoPreview} alt="선택한 프로필 사진" /> : <span>＋ 프로필 사진</span>}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { selectChildPhoto(event.currentTarget.files?.[0]); event.currentTarget.value = '' }} /></label><label className="form-label">이름</label><input className="form-control" value={childNameInput} onChange={event => setChildNameInput(event.target.value)} placeholder="아이 이름" /><label className="form-label">생년월일</label><input className="form-control" type="date" max={new Date().toLocaleDateString('sv-SE')} value={childBirthDateInput} onChange={event => setChildBirthDateInput(event.target.value)} />{childBirthDateInput && <p className="child-age-preview">현재 {ageFromBirthDate(childBirthDateInput)}</p>}<label className="form-label">학교·기관</label><input className="form-control" value={childInstitutionInput} onChange={event => setChildInstitutionInput(event.target.value)} placeholder="예: 한빛초등학교, 별빛유치원" /><button className="outline-button wide-button" onClick={addOnboardChild}>＋ 자녀 추가</button></Card><Card className="academy-optional"><strong>일정은 가족방에서 등록해요</strong><p>가족방에 들어간 뒤 캘린더의 ＋ 버튼에서 아이 일정과 루틴을 추가할 수 있어요.</p></Card><button className="primary-button wide-button onboard-next" disabled={onboardBusy} onClick={() => void saveOnboardChildren()}>{onboardBusy ? '등록 중…' : '다음'}</button></div>
  ...
  if (boot && screen === 'calendar') page = <section className="calendar-management-page">
    <div className="eyebrow">개인 캘린더 연동 및 루틴 수정</div>
    <h2 className="hero-title">내 일정과 루틴을<br />한곳에서 관리해요</h2>
    <p className="hero-copy">외부 캘린더 연결 상태를 확인하고, 내가 직접 등록한 개인 루틴을 수정할 수 있어요.</p>
