...
function App() {
  ...
  const timeline = (list: Assignment[]) => {
    ...
    return <Card className="timeline-card">{[...currentByItem.values()].sort((a, b) => (itemFor(a)?.starts_at || '').localeCompare(itemFor(b)?.starts_at || '')).map(a => { const i = itemFor(a); const statusClass = a.status === 'COMPLETED' ? 'status-done' : ['PROPOSED', 'CANDIDATE_ACCEPTED'].includes(a.status) ? 'status-pending' : 'status-progress'; return i && <button key={a.id} className="timeline-row" onClick={() => { setAssignmentId(a.id); setViewer(a.assignee_id); go('assignmentDetail') }}><span className="time">{formatTime(i.starts_at) || '—'}</span><span className="timeline-content"><strong>{i.title} — {a.status === 'PROPOSED' ? `${member(a.assignee_id)}님에게 요청` : a.status === 'CANDIDATE_ACCEPTED' ? `${member(a.assignee_id)}님 수락` : `담당 ${member(a.assignee_id)}`}</strong><small className={statusClass}>{child(i.child_id)} · {caregiverStatusLabel(a.status)}</small></span><span className="timeline-status">{a.status === 'COMPLETED' ? '✓' : '›'}</span></button> })}</Card>
  }
