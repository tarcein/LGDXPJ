...
await page.addInitScript(() => {
  ...
  Object.defineProperty(navigator, 'share', { configurable: true, value: async data => { window.__albumSharedImage = data?.files?.[0] instanceof File && data.files[0].type.startsWith('image/') } })
})
...
try {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (joined && !publicInvitePreview && !['/api/families', '/api/families/join', '/api/families/dev-login-options'].includes(path) && request.headers().authorization !== 'Bearer qa-session') throw new Error(`${path}: 가족방 토큰 누락`)
    let body
    ...
    else if (path === '/api/assistant/chat' && method === 'POST') { chatSent = request.postDataJSON().message === '등록된 일정은?'; body = { message: '등록된 일정은?', answer: '서버 AI 답변', cards: [], links: [], schedule_changes: [], schedule_creations: [], usage: { total_tokens: 30, used_today: 30, limit: 50000, remaining: 49970 }, plan: 'FREE' } }
    else if (path === '/api/assistant/voice' && method === 'POST') { voiceUploaded = request.headers()['content-type']?.startsWith('multipart/form-data; boundary='); body = { transcript: '음성 질문', message: '음성 질문', answer: '음성 AI 답변', cards: [], links: [], schedule_changes: [], usage: { total_tokens: 35, used_today: 65, limit: 50000, remaining: 49935 }, plan: 'FREE' } }
    ...
    else if (path === '/api/features') { const limit = family.plan === 'PRO' ? 200000 : 50000; body = { plan: family.plan, features: [], usage: { chat_tokens_today: 0, chat_tokens_limit: limit, chat_tokens_remaining: limit, ocr_today: 0 } } }
  ...
  await page.locator('.thinq-entry').waitFor()
  await page.setViewportSize({ width: 360, height: 800 })
  if (await page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth) > 1) throw new Error('360px ThinQ 홈이 가로로 넘쳐요')
  await page.screenshot({ path: join(tmpdir(), 'lgdx-mobile-thinq.png') })
  ...
  await page.setViewportSize({ width: 1200, height: 840 })
  await page.getByRole('button', { name: '홈 선택 닫기' }).click()
  await page.locator('.thinq-home-picker').click()
  await page.getByRole('button', { name: /가족방 지금 만들기/ }).click()
  if (await page.locator('.onboarding-art, .onboard-circle, .onboard-line').count()) throw new Error('온보딩에 엄·할·아 장식이 남아 있어요')
  ...
  await page.getByRole('button', { name: '등록된 일정은?' }).click()
  await page.getByText('서버 AI 답변').waitFor()
  await page.getByText('49,970').waitFor()
  ...
  for (const label of ['ThinQ 홈', '잠금화면 동선', '홈', '알림장·돌봄 정보', '추출 결과 확인', '오늘의 배정', '배정 제안', '오늘 할 일', '개인 일정', '예외 상황', '알림함', '온보딩', '캘린더 연동', '가족 구성원', '정보 공개 권한', '알림 설정', '플랜 비교', 'AI 채팅', '긴급 요청', '돌봄 동선', '돌봄 공백 예측', '모음ZIP', '돌봄 제도']) {
    ...
    if (label === '예외 상황') {
      await page.getByRole('heading', { name: /현재 예외 상황이/ }).waitFor()
      if (await page.getByText('일정 충돌 감지').count()) throw new Error('실제 예외가 없는데 하드코딩된 충돌 경고가 표시돼요')
    }
    if (label === '캘린더 연동') await page.getByRole('heading', { name: /개인 일정을 편하게 연동해보세요/ }).waitFor()
  ...
  await page.getByText('내가 맡은 일').waitFor()
  if (Number.parseFloat(await page.locator('.section-heading').first().evaluate(element => getComputedStyle(element).fontSize)) < 14) throw new Error('모바일 본문 제목 글씨가 다시 작아졌어요')
  ...
  if (await page.locator('.mobile-status').isVisible()) throw new Error('실제 모바일 화면에 목업 상태바가 중복 표시돼요')
  if (await page.locator('.phone').evaluate(element => getComputedStyle(element).borderRadius) !== '0px') throw new Error('실제 모바일 화면에 목업 프레임 모서리가 남아 있어요')
  if (await page.locator('.floating-assistant').count() !== 1) throw new Error('서비스 화면에 플로팅 어시스턴트가 없어요')
  ...
  await page.locator('.figma-calendar-card').waitFor()
  ...
  const calendarLayout = await page.locator('.phone').evaluate(phone => {
    const card = phone.querySelector('.figma-calendar-card')
    const day = phone.querySelector('.figma-month-grid button:not(.muted)')
    return { phoneWidth: phone.getBoundingClientRect().width, cardWidth: card?.getBoundingClientRect().width, dayHeight: day?.getBoundingClientRect().height }
  })
  if (!calendarLayout.cardWidth || calendarLayout.cardWidth < calendarLayout.phoneWidth - 1 || (calendarLayout.dayHeight ?? 0) < 50) throw new Error('Figma 일정10처럼 달력이 화면을 꽉 채우지 않아요: ' + JSON.stringify(calendarLayout))
  await page.locator('.figma-calendar-agenda').getByText('태권도').waitFor()
  await page.getByRole('button', { name: /개인 캘린더 연동/ }).waitFor()
  if (await page.getByRole('button', { name: '업무 캘린더 연결' }).count()) throw new Error('일정 화면에 연동 버튼처럼 보이는 뒤로가기가 남아 있어요')
  if (await page.locator('.figma-calendar-agenda > button').count() !== 4) throw new Error('하루 일정 4개가 모두 표시되지 않아요')
  await page.locator('.figma-month-grid .month-day-events small').getByText('+1').waitFor()
  await page.getByRole('button', { name: '등록 메뉴 열기' }).click()
  await page.getByRole('heading', { name: '무엇을 등록할까요?' }).waitFor()
  await page.getByRole('button', { name: /일정 등록/ }).waitFor()
  await page.getByRole('button', { name: /루틴 등록/ }).waitFor()
  ...
  await page.locator('.figma-calendar-agenda').getByText('태권도').click()
  await page.getByText('선택한 날짜').waitFor()
  await page.locator('.schedule-day-sheet').getByText('태권도').waitFor()
  await page.locator('.schedule-day-sheet').getByRole('button', { name: '수정' }).first().click()
  ...
  await page.getByRole('dialog', { name: '일정 등록' }).waitFor()
  if (await page.locator('.single-schedule-screen .routine-repeat-tabs').count()) throw new Error('일반 일정 등록에 반복 주기 UI가 표시돼요')
  const singleTimes = page.locator('.single-schedule-screen .direct-time input')
  if (await singleTimes.nth(0).getAttribute('inputmode') !== 'numeric') throw new Error('일반 일정 시작 시간 직접 입력 키보드 설정이 없어요')
  if (await singleTimes.nth(1).getAttribute('placeholder') !== '없으면 비워두기') throw new Error('일반 일정 종료 시간 선택 입력 안내가 없어요')
  await singleTimes.nth(0).fill('930')
  await singleTimes.nth(0).press('Tab')
  if (await singleTimes.nth(0).inputValue() !== '09:30') throw new Error('930 시간이 09:30으로 정규화되지 않았어요')
  await page.screenshot({ path: join(tmpdir(), 'lgdx-mobile-schedule-single-form.png'), fullPage: true })
  await page.getByRole('button', { name: '일정 등록 닫기' }).click()
  await page.getByRole('heading', { name: '누구의 일정인가요?' }).waitFor()
  await page.getByRole('button', { name: '돌아가기' }).click()
  await page.getByRole('heading', { name: '무엇을 등록할까요?' }).waitFor()
  await page.getByRole('button', { name: /루틴 등록/ }).click()
  await page.getByRole('dialog', { name: '루틴 등록' }).waitFor()
  if (await page.locator('.routine-progress, .routine-intro').count()) throw new Error('루틴 등록에 온보딩 단계 안내가 남아 있어요')
  await page.getByRole('button', { name: '루틴 등록 닫기' }).click()
  await page.getByRole('dialog', { name: '루틴 등록' }).waitFor({ state: 'hidden' })
  if (await page.getByRole('heading', { name: '무엇을 등록할까요?' }).count()) throw new Error('루틴 등록 닫기가 이전 선택창으로 돌아가요')
  await page.getByRole('button', { name: '등록 메뉴 열기' }).click()
  await page.getByRole('button', { name: /루틴 등록/ }).click()
  await page.getByRole('dialog', { name: '루틴 등록' }).waitFor()
  if (await page.locator('.routine-screen .mobile-status').count()) throw new Error('루틴 등록 화면에 기기 상태바가 중복 표시돼요')
  const directTimes = page.locator('.routine-screen .direct-time input')
  ...
  await page.getByRole('button', { name: 'N일마다', exact: true }).click()
  await page.getByRole('spinbutton', { name: '반복 간격' }).fill('3')
  await page.getByRole('button', { name: '월간', exact: true }).click()
  await page.getByRole('spinbutton', { name: '매월 반복일' }).fill('15')
  await page.getByRole('button', { name: '특정일', exact: true }).click()
  await page.getByLabel('특정 반복일').fill(new Date(Date.now() + 86400000).toLocaleDateString('sv-SE'))
  await page.locator('.routine-specific-dates').getByRole('button', { name: '추가' }).click()
  if (await page.locator('.routine-specific-dates > p button').count() !== 1) throw new Error('특정일 반복 날짜가 추가되지 않았어요')
  await page.getByRole('button', { name: '요일', exact: true }).click()
  const repeatSwitch = page.getByRole('switch', { name: '매주 반복' })
  if (await repeatSwitch.getAttribute('aria-checked') !== 'true') await repeatSwitch.click()
  await page.getByText('반복 종료일').waitFor()
  const routineLayout = await page.locator('.routine-screen').evaluate(screen => {
    const card = screen.querySelector('.routine-form-card')
    return { screenBottom: screen.getBoundingClientRect().bottom, cardBottom: card?.getBoundingClientRect().bottom }
  })
  if (!routineLayout.cardBottom || routineLayout.screenBottom - routineLayout.cardBottom > 24) throw new Error('루틴 등록 폼 아래에 불필요한 빈 공간이 남아 있어요: ' + JSON.stringify(routineLayout))
  ...
  await page.screenshot({ path: join(tmpdir(), 'lgdx-mobile-care.png') })
  await page.getByRole('button', { name: /내 돌봄·완료/ }).click()
  if (await page.locator('.app-header').count()) throw new Error('세부 기능 화면에 가족방 전환 헤더가 남아 있어요')
  ...
  const homeHeader = await page.locator('.app-header').evaluate(element => ({ text: element.textContent, rect: element.getBoundingClientRect().toJSON(), display: getComputedStyle(element).display, opacity: getComputedStyle(element).opacity }))
  if (!homeHeader.text?.includes(family.name) || homeHeader.rect.y < 0 || homeHeader.display !== 'flex' || homeHeader.opacity !== '1') throw new Error('홈 Figma 헤더가 정상 배치되지 않았어요: ' + JSON.stringify(homeHeader))
  if (await page.locator('.success-toast').count()) await page.locator('.success-toast').waitFor({ state: 'hidden' })
  ...
  for (const viewport of [{ width: 360, height: 800 }, { width: 430, height: 932 }]) {
    ...
    if (overflow > 1) throw new Error(`${viewport.width}px 홈 화면이 가로로 ${overflow}px 넘쳐요`)
  }
  await page.setViewportSize({ width: 390, height: 844 })
  await page.screenshot({ path: join(tmpdir(), 'lgdx-mobile-home.png') })
  ...
  await page.locator('.app-header').waitFor({ state: 'visible' })
  await page.waitForTimeout(200)
  const familyChrome = await page.locator('.phone').evaluate(phone => {
    const status = phone.querySelector('.mobile-status')
  ...
  if (familyChrome.statusVisible || !familyChrome.header || familyChrome.header.y < familyChrome.phone.y) throw new Error('가족 화면 모바일 헤더 배치가 잘못됐어요: ' + JSON.stringify(familyChrome))
  if (await page.locator('.success-toast').count()) await page.locator('.success-toast').waitFor({ state: 'hidden' })
  ...
  const saveAlbumPhoto = page.getByRole('button', { name: '사진 앱에 저장' })
  await saveAlbumPhoto.waitFor()
  await saveAlbumPhoto.click()
  if (!await page.evaluate(() => window.__albumSharedImage === true)) throw new Error('앨범 사진이 iPhone 공유 시트에 이미지 파일로 전달되지 않아요')
  page.once('dialog', dialog => dialog.accept())
  ...
  await page.getByText('혜택·부가서비스').waitFor()
  if (await page.locator('.success-toast').count()) await page.locator('.success-toast').waitFor({ state: 'hidden' })
  await page.screenshot({ path: join(tmpdir(), 'lgdx-mobile-more.png') })
  ...
  await page.getByRole('heading', { name: /가족의 돌봄을/ }).waitFor()
  if (await page.evaluate(() => localStorage.getItem('family-care-access-token'))) throw new Error('로그아웃 뒤 가족방 토큰이 남아 있어요')
